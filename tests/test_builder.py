...
class TestSpecialCronJob(unittest.TestCase):

    def test_setters_and_getters(self):
        job = Job()
        job.expression = '@daily'
        job.command = 'ls'
        job.log_file = 'out.log'
        job.error_log_file = 'err_out.log'
        job.comments = 'Hey this is a comment'
        ...
        try:
            ...
            print "No Hour Required"

        try:
            self.assertEquals(job.day_of_month, '*')
        except IndexError:
            print "No Day of Month Required"

        try:
            self.assertEquals(job.month, '*')
        except IndexError:
            print "No Month Required"

        try:
            self.assertEquals(job.day_of_week, '*')
        except IndexError:
            print "No Day of Week Required"
    ...
    def test_parse_job_line(self):
        line = "@daily ls >> out.log 2>> err_out.log"
        job = Job()
        job.line = line

        # test expression part
        self.assertEquals(job.expression, '@daily')
    ...
    def test_job_line(self):
        line = "@daily ls >> out.log"
        job = Job(line)
        self.assertEquals(job.line, line)
        self.assertEquals(str(job), line)

class TestSpecialCronJobs(unittest.TestCase):

    def setUp(self):
        self.job1 = Job("@daily job1")
        self.job2 = Job("@daily job2")
        self.job3 = Job("@daily job3")
        self.job4 = Job("@daily job4")
        self.job5 = Job("@daily job5")
